    ...
    <TableBody sx={{ fontFamily: "'Helvetica', 'Arial', sans-serif" }}>
      {typesData.map((row) => (
          <TableRow key={row.ID} sx={{
            ...
            backgroundColor: (temp[row.ID-1].spending === 0 && temp[row.ID-1].amount === 0) ?  {backgroundColor:"#F5E8E8"}: (temp[row.ID-1].difference > 0  ? "#92F685" : "#FF9292"),
            ...
            <TableCell align="justify">{temp[row.ID-1].amount}</TableCell>
            <TableCell align="justify">{temp[row.ID-1].spending}</TableCell>
            <TableCell align="justify">{temp[row.ID-1].difference}</TableCell>
        </TableRow>
      ))}
    </TableBody>
  </Table>
